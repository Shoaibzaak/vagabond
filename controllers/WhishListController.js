...
module.exports = {
  ...
  updateWhishlist: catchAsync(async (req, res, next) => {
    ...
    try {
      // Check if the "isVisited" parameter is true in the request params
      if (req.query.isVisited === "true") {
        const updatedWhishlist = await Model.Whishlist.findOneAndUpdate(
        ...
        WhishlistUserData.isVisited = true;
      }
  ...
  declineWhishlist: catchAsync(async (req, res, next) => {
    var WhishlistId = req.params.id;
    try {
      const WhishlistUser = await Model.Whishlist.findByIdAndDelete({
        _id: WhishlistId,
      });
      if (!WhishlistUser)
